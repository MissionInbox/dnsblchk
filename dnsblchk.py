...
def dnsblk_handler(servers, ips):
    ...
    if shutdown == False:
        try:
            ...
            print("Checking " + str(len(ips)) + " IP addresses")
                ...
                for ip in ips:
                    ...
                    if ret is not False:
                        if log_file_handler is None:
                            log_file_handler = open(dnsblk_log + str(int(time.time())) + ".log", 'w')
                            csvwriter = csv.writer(log_file_handler, delimiter=',', quotechar='"',
                                                   quoting=csv.QUOTE_ALL)

                        csvwriter.writerow(
                            [time.strftime("%d %b %Y %H:%M:%S", time.gmtime()), ip[0], server[0], ret[1]])
                        ...
                        print("DIRTY : " + ip[0] + " LISTED IN " + server[0])
                    else:
                        print("clean : " + ip[0] + " unlisted in " + server[0])

                    # sleep for a short time after each query
                    time.sleep(0.01)

            print("Listed " + str(len(listed_ips)) + " IP addresses")
            if log_file_handler is not None:
                log_file_handler.close()

            # If there are listed IP addresses, notice each administrator by email
            if len(listed_ips) > 100000: # TODO Enable back.
...
if __name__ == "__main__":
    ...
    while True:
        ...
        try:
            ...
            break # TODO Remove break to enable run as a service.
